...
export const SectionGuideModal: React.FC<SectionGuideModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        className="relative w-full max-w-3xl max-h-[90vh] bg-white rounded-3xl border border-purple-200 shadow-2xl shadow-purple-950/15 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-5 sm:p-6 bg-gradient-to-r from-[#FAF8FD] via-[#FAF8FD] to-[#FAF8FD] border-b border-purple-100 flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-100/80 border border-purple-200 text-purple-800 text-xs font-bold uppercase tracking-wider">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-full text-neutral-400 hover:text-neutral-700 hover:bg-purple-100/50 transition-colors"
            title="Cerrar guía"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto p-5 sm:p-7 space-y-6 text-[#241235] text-sm">
          {/* Plan Básico Notice */}
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-base font-serif text-[#241235] flex items-center gap-2">
            ...
            <div className="space-y-2.5">
              {steps.map((st) => (
                <div 
                  key={st.step}
                  className="p-3.5 rounded-2xl bg-[#FAF8FD] border border-purple-100/90 flex items-start gap-3 hover:border-purple-300 transition-colors"
                >
                  <span className="w-6 h-6 rounded-full bg-purple-700 text-white font-bold text-xs flex items-center justify-center shrink-0 shadow-xs">
                    {st.step}
                  </span>
          ...
          <div className="space-y-3">
            ...
            <div className="divide-y divide-rose-100 rounded-2xl border border-purple-100 overflow-hidden bg-white">
              {fields.map((f, idx) => (
                <div key={idx} className="p-3.5 hover:bg-purple-50/30 transition-colors flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4">
                  <div className="sm:w-1/3 shrink-0">
                    <span className="font-bold text-xs text-purple-800 flex items-center gap-1.5">
                      {f.name}
                      {f.required && (
                        <span className="text-[10px] text-purple-600 font-semibold">(Obligatorio)</span>
                      )}
                    </span>
                  </div>
        ...
        {/* Footer */}
        <div className="p-4 bg-[#FAF8FD] border-t border-purple-100 flex items-center justify-between">
          <span className="text-xs text-neutral-400 italic">
            Studio Foráneas • Documentación Interna
          </span>
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-500 hover:to-violet-500 text-white font-semibold text-xs tracking-wide shadow-md shadow-purple-500/20 transition-all cursor-pointer"
          >
