...
import { ZoomIn, ChevronLeft, ChevronRight, LayoutGrid, Sliders, Sparkles } from 'lucide-react';
...
export const PortfolioSection: React.FC<PortfolioSectionProps> = ({ items, onSelectPhoto, config }) => {
  ...
  return (
    <section id="portafolio" className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12">
      {/* Section Title */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-3 max-w-2xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-purple-100/90 border border-purple-200 text-purple-800 text-xs font-bold uppercase tracking-widest shadow-2xs">
            ...
            <span>{config?.portafolioBadge || 'Portafolio Visual'}</span>
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold font-serif text-[#241235] tracking-tight">
            {config?.portafolioTitulo || 'Nuestros Trabajos Realizados'}
          </h2>
          <p className="text-[#554064] text-sm sm:text-base">
            {config?.portafolioSubtitulo || 'Fotografía documental de bodas, campañas comerciales para marcas y contenido audiovisual dinámico que conecta con la emoción y la estética.'}
          </p>
        </div>

        {/* View Mode Switcher */}
        <div className="flex items-center gap-1.5 self-start md:self-auto bg-white p-1 rounded-2xl border border-purple-100 shadow-xs">
          <button
            onClick={() => setViewMode('grid')}
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
              viewMode === 'grid' ? 'bg-purple-50 text-purple-800 shadow-xs' : 'text-[#725C80] hover:text-[#241235]'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
              viewMode === 'carousel' ? 'bg-purple-50 text-purple-800 shadow-xs' : 'text-[#725C80] hover:text-[#241235]'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer ${
              activeCategory === cat.id
                ...
                : 'bg-white text-[#554064] hover:text-purple-700 border border-purple-100'
            }`}
          >
            {cat.label}
          </button>
      ...
      {viewMode === 'carousel' && filteredItems.length > 0 && (
        <div className="relative rounded-3xl overflow-hidden bg-stone-900 border border-purple-200 group shadow-xl">
          <div className="relative aspect-[16/9] sm:aspect-[21/9] w-full max-h-[600px] overflow-hidden">
            ...
            <div className="absolute bottom-6 left-6 right-6 sm:bottom-10 sm:left-10 sm:right-10 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 pointer-events-auto">
              <div className="space-y-2 max-w-2xl">
                <span className="px-3 py-1 rounded-full bg-lime-400 text-purple-950 text-[11px] font-extrabold uppercase tracking-wider">
          ...
          {filteredItems.length > 1 && (
            <>
              <button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/85 hover:bg-white text-stone-800 shadow-md backdrop-blur-md transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/85 hover:bg-white text-stone-800 shadow-md backdrop-blur-md transition-all cursor-pointer"
              >
                ...
                {filteredItems.map((_, idx) => (
                  <button
                    ...
                    className={`h-1.5 rounded-full transition-all cursor-pointer ${
                      idx === carouselIndex ? 'w-6 bg-lime-400' : 'w-1.5 bg-white/60'
                    }`}
                  />
                ))}
              </div>
          ...
          {filteredItems.map((item) => {
            ...
            return (
              <div
                ...
                className="group relative aspect-[4/3] rounded-3xl overflow-hidden bg-purple-50/50 border border-purple-100 hover:border-purple-300 hover:shadow-xl hover:shadow-purple-900/10 cursor-pointer shadow-sm transition-all duration-300"
              >
                <img
                  src={directImg}
                  alt={item.titulo}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#200E2E]/90 via-[#200E2E]/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-6 flex flex-col justify-end">
                  <div className="space-y-1.5">
                    <span className="inline-block px-3 py-0.5 rounded-full bg-lime-400 text-purple-950 text-[10px] font-extrabold uppercase tracking-wider">
                      {item.categoria}
                    </span>
      ...
      {filteredItems.length === 0 && (
        <div className="text-center py-16 bg-white rounded-3xl border border-purple-100 shadow-sm">
          <p className="text-[#725C80] text-sm">No hay fotografías disponibles en esta categoría.</p>
        </div>
      )}
    </section>
  );
